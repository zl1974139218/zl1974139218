...
public class StudentServlet extends HttpServlet {
	...
	private void search(HttpServletRequest request, HttpServletResponse response) {
		...
		List<Student> list = stuDao
				.searchByCondition(condition, begin, pageNum);
	...
	private void modify(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			int bjId = 0;
			String newFileName = "";
			FileItemFactory factory = new DiskFileItemFactory();// 为请求创建一个DiskFileItemFactory对象，通过它来解析请求。执行解析后，所有的表单项目都会存在一个List中。
			...
			for (int i = 0; i < items.size(); i++) {
				if ("id".equals(items.get(i).getFieldName())) {
					...
					newFileName = uuid + houzhui;
						...
						//id = Integer.parseInt(request.getParameter("selectId"));
						StudentDao stuDao = new StudentDao();
						Student stu = stuDao.searchById(id);
						newFileName=stu.getPhoto();
					}
